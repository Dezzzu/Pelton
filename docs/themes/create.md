...
Your theme's folder (`README.md`, `LICENSE`, `my-theme.peltontheme`, `preview.png`) needs to land inside a `themes/my-theme/` folder in a clone of [TRC-Loop/pelton-themes](https://github.com/TRC-Loop/pelton-themes), the gallery repo, alongside `source/`:

```bash
git clone https://github.com/TRC-Loop/pelton-themes.git
...
- **Issue form**: open a ["Submit a theme"](https://github.com/TRC-Loop/pelton-themes/issues/new?template=submit_theme.yml) issue and attach your files, a maintainer turns it into a PR for you. GitHub won't let you attach a `.peltontheme` file directly to an issue, since it's already a zip, rename it to `.zip` before attaching (or drop it inside a new `.zip`), the repo's CI renames it back automatically once it lands.
